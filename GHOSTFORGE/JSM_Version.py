...
def initialize_genesis(dag, blue_set, blue_scores):
    ...
    dag.add_node('Genesis')  # Add Genesis block to DAG
    blue_set.add('Genesis')  # Add Genesis block to the blue set
    blue_scores['Genesis'] = 1  # Set the initial score for the Genesis block
...
def block_level_coloring_and_scoring(dag, block, red_set, blue_set, blue_scores, k, visualizer):
    ...
    current_blue_set = blue_set.copy()
    current_red_set = {node for node in dag.nodes if node not in current_blue_set}
    current_blue_scores = blue_scores.copy()

    # Visualize the DAG after each block-level coloring and scoring
    visualizer.visualize_dag_after_block_level(block, current_blue_set, current_red_set, current_blue_scores)
...
def dag_level_coloring_and_ordering(dag, blue_set, red_set, blue_scores, k, previous_order):
    ...
    def check_and_add_to_blue_set(tip, inherited_blue_set):
        ...
        if len(blue_anticone) <= k:
            ...
            print(f"Extended blue set to check check the aniticones: {inherited_blue_set}")
...
def apply_J_version(dag, new_blocks, blue_set, red_set, blue_scores, visualizer, previous_order, k=3): # wait_for_input=True,
