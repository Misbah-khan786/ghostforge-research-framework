...
def dag_level_coloring_and_ordering(dag, blue_set, red_set, blue_scores, k, previous_order, miner_id, block_count,
                                    locked_order):
    ...
    # max_tip = max(tips_list, key=lambda x: (blue_scores.get(x, 0), -ord(x[0])))
    ...
    # Update block count with the total number of blocks in the DAG (excluding Genesis)
    block_count = len(dag.nodes) - 1  # Subtract 1 to exclude the Genesis block
    # print(f"Updated Block Count: {block_count}")

    threshold = 10
    # Determine the final order
    if block_count % threshold == 0:
        # Sort based on the score with red blocks at the end
        print(f"ORDER after Score based sorting")
        sorted_blocks = sorted(dag.nodes, key=lambda x: (blue_scores.get(x, float('inf')), x))
        new_order = [block for block in sorted_blocks if block not in red_set] + [block for block in sorted_blocks if
                                                                                  block in red_set]

        # Lock this order for the next interval
        previous_order[:] = new_order  # Update the previous order in-place
        locked_order = True  # Set flag to indicate that order is locked
    else:
        # Maintain the locked order and append new blue blocks to the right
        print(f"ORDER MAINTAINED as usual")
        if locked_order:
            ...
            new_order = previous_order.copy()  # For the first iteration, use previous order
            new_order += [block for block in additional_blue_set if block not in new_order]
        new_order += sorted(red_set, key=lambda x: blue_scores.get(x, 0), reverse=False)

    # Debugging information
    print(f"Final Ordered List (including red blocks at the end) for MINER-ID {miner_id}: {new_order} ")
...
def update_scores(dag, blue_set, red_set, blue_scores):
    ...
    for block in nx.topological_sort(dag):
        ...
        # print(f"Updated score for block {block}: {blue_score}")


def apply_ghostforge(dag, new_blocks, blue_set, red_set, blue_scores, visualizer, previous_order, miner_id, block_count,
                     locked_order, k=3):  # wait_for_input=True,
