...
from GHOSTFORGE.GF_ScoreOrder_DY import initialize_genesis, apply_ghostforge, update_scores
from Network.Delaymatrix import MinerDelayMatrix  # Assuming you have the MinerDelayMatrix class in a separate file
from DAGVisulizer.dagvisualizerScoreOrder_DY import DAGVisualizer
...
from Simulation.Order_Convergence_SCOREORDERING import (record_orders, update_orders, check_global_order_convergence,
                                                        plot_global_convergence, check_individual_order_convergence,
                                                        plot_individual_convergence, check_threshold_order)
...
class Miner:
    ...
    def generate_block(self, step):
        ...
        if current_time >= self.next_generation_time:
            ...
            self.next_generation_time = current_time + random.randint(self.min_gen_time_ms,
                                                                      self.max_gen_time_ms)  # Next generation time
        else:
    ...
    def receive_block(self, block_id, parents):
        ...
        time_module.sleep(random.uniform(15, 30) / 1000)  # Reception handling delay
    ...
    def receive_block_immediately(self, block_id, parents):
        ...
        if block_id not in self.dag:
            ...
            for parent in parents:
                self.dag.add_edge(parent, block_id)
            self.apply_protocol()  # Apply protocol on receiving the block
            # self.visualize_current_dag(block_id, "master_after_receiving")  # Visualize the DAG after receiving the
            # block
...
def simulate_miners(miners, master_miner, num_time_steps, step_duration_ms, generate_prob=0.9):
    ...
    for t in range(1, num_time_steps + 1):
        ...
        # # Check orders at threshold points
        # check_threshold_order(miners, threshold)
    ...
    output_dir = 'SO_global_individual_convergence'
    ...
    # output_dir-7-30-35 = 'S0_global_individual_convergence'
    # # Call the plot functions
    # plot_global_convergence(common_prefixes, output_dir-7-30-35)
    # plot_individual_convergence(individual_convergence, output_dir-7-30-35)
...
if __name__ == "__main__":
    ...
    step_duration_ms = 100  # Step duration in milliseconds
