...
from GHOSTFORGE.GF_ReorderEveryTime import initialize_genesis, apply_ghostforge, update_scores
from Network.Delaymatrix import MinerDelayMatrix  # Assuming you have the MinerDelayMatrix class in a separate file
from DAGVisulizer.ReorderEveryTime import DAGVisualizer
from Simulation.order_convergence import (record_orders, update_orders, check_partial_order_convergence,
                                          check_global_order_convergence, plot_global_convergence,
                                          plot_individual_convergence, check_individual_order_convergence,
                                          plot_partial_order_convergence)
...
class Miner:
    ...
    def visualize_current_dag(self, block_id=None, context=""):
        ...
        output_dir = f'output_directory_ET/{self.miner_id}'
...
def simulate_miners(miners, master_miner, steps):
    ...
    for t in range(len(steps)):
        ...
        # input(f"Start Propagation at time step t{t}...")
            ...
            if t == 1:
                ...
                miners[1].propagate_blocks([miners[3], miners[0], miners[2]])  # M2 sends to M4
                miners[2].propagate_blocks([miners[1], miners[0], miners[3]])  # M3 sends to M2 and M4
    ...
    partial_convergence = check_partial_order_convergence(miner_orders)
    global_convergence_step, longest_common_prefix, common_prefixes = check_global_order_convergence(miner_orders)
    individual_convergence = check_individual_order_convergence(miner_orders)
    plot_partial_order_convergence(partial_convergence)


    print("Partial Order Convergence Points:")
    for miner_id, (step, length, prefix) in partial_convergence.items():
        print(f"Miner {miner_id}: Convergence Step {step}, Longest Constant Prefix Length {length}, Prefix {prefix}")
    print(f"Global Convergence Point: {global_convergence_step}, Longest Common Prefix: {longest_common_prefix}")
    ...
    plot_global_convergence(common_prefixes)
    plot_individual_convergence(individual_convergence)
